function [ output ] = BQX_Project1FINAL_v2 ( fleetSize )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while time < simulationTime
            ...
            %Passenger moving backwards
                ...
                if (pos > stopsPosition(shuttlePosition(fasterShuttle(1),3)))
                    ...
                    elseif (pos == stopsPosition(event(1,2)))
                        ...
                        if (abs((event(1,3)) - pos) < abs(stopsPosition(shuttlePosition(fasterShuttle(1),3)) - pos))
                            shuttlePosition(fasterShuttle(1),3) = event(1,3); 
                        end                            
                    end
                %Shuttle heading forwards
    ...
    if (event(1,1) == 2)
        ...
        if (shuttleNumberPass(1,event(1,2)) == 0)
            ...
            cont = zeros(1, numberOfStops); %counter of the number of shuttles per garage
            for i=1:numberShuttles
                for j=1:numberOfStops
                    pos = shuttlePosition(i,1) + shuttlePosition(i,4)*shuttleAverageSpeed*(time - shuttlePosition(i,2));
                    if(pos == stopsPosition(j) && shuttleNumberPass(1,i) == 0)
                        cont(1,j) = cont(1,j) + 1;
                    elseif (shuttlePosition(i,3) == j && shuttleNumberPass(1,i) == 0)
                        cont(1,j) = cont(1,j) + 1;
                    end
                end
            end
            nextDest = 1;
            contaux = stopsPosition(numberOfStops);
            for i=1:numberOfStops
                if (cont(1,i) <= cont(1,nextDest))
                    if (cont(1,i) < cont(1,nextDest))
                        contaux = abs(stopsPosition(shuttlePosition(event(1,2),3)) - stopsPosition(i));
                        nextDest = i;
                    elseif (abs(stopsPosition(shuttlePosition(event(1,2),3)) - stopsPosition(i)) < contaux)
                        contaux = abs(stopsPosition(shuttlePosition(event(1,2),3)) - stopsPosition(i));    
                        nextDest = i;
                    end
                end
